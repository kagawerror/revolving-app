...
void main() {
  ...
  test('watchByStatusAll returns pending docs across all companies, excluding other statuses',
      () async {
    await seed('a', companyId: 'c1', status: 'pendingAck', createdAt: DateTime(2026, 1, 1));
    await seed('b', companyId: 'c2', status: 'pendingAck', createdAt: DateTime(2026, 1, 2));
    ...
    final list = await repo.watchByStatusAll(RequestStatus.pendingAck).first;
    final ids = list.map((r) => r.id).toSet();
    expect(ids, {'a', 'b'});
    expect(list.every((r) => r.status == RequestStatus.pendingAck), isTrue);
  });
